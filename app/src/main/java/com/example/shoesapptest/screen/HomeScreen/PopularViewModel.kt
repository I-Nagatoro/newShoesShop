package com.example.shoesapptest.screen.HomeScreen
...
class PopularViewModel(
    ...
    private val _sneakersState =
        MutableStateFlow<NetworkResponseSneakers<List<PopularResponse>>>(Loading)
    val sneakersState: StateFlow<NetworkResponseSneakers<List<PopularResponse>>> = _sneakersState
    private val _favoritesState =
        MutableStateFlow<NetworkResponseSneakers<List<PopularResponse>>>(Loading)
    val favoritesState: StateFlow<NetworkResponseSneakers<List<PopularResponse>>> = _favoritesState
    ...
    fun toggleFavorite(sneakerId: Int, wasFavoriteBeforeClick: Boolean) {
        viewModelScope.launch {
            ...
            (_sneakersState.value as? Success<List<PopularResponse>>)?.let { successState ->
            ...
            (_favoritesState.value as? Success<List<PopularResponse>>)?.let { successState ->
                if (newFavorite) {
                    val added = (_sneakersState.value as? Success<List<PopularResponse>>)
                        ?.data
